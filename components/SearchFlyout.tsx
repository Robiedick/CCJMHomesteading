    ...
    <div className="fixed right-4 top-4 z-50 flex justify-end sm:right-8 sm:top-6">
      ...
      >
        <button
          ...
          aria-expanded={open}
          className="flex h-12 w-12 items-center justify-center rounded-xl border-2 border-gray-200 bg-white text-gray-700 shadow-lg shadow-gray-900/10 transition-all duration-200 hover:-translate-y-1 hover:border-emerald-300 hover:text-emerald-600 hover:shadow-xl hover:shadow-emerald-500/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500"
        >
          <Search className="h-5 w-5" strokeWidth={2.5} />
          <span className="sr-only">{labels.title}</span>
        </button>
        <div className={panelClasses} role="dialog" aria-label={labels.title}>
          <div className="max-h-[72vh] overflow-y-auto rounded-2xl border-2 border-gray-200 bg-white p-8 shadow-2xl shadow-gray-900/15">
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className="text-xl font-bold text-gray-900">{labels.title}</h2>
                {!hasActiveQuery ? (
                  <p className="mt-2 text-sm text-gray-600">{labels.minimumCharactersMessage}</p>
                ) : null}
              </div>
              {hasActiveQuery ? (
                <button
                  type="button"
                  onClick={handleClear}
                  className="text-sm font-bold text-emerald-600 transition-colors duration-200 hover:text-emerald-700"
                >
                  {labels.clearLabel}
                </button>
              ) : null}
            ...
            <form className="mt-6 space-y-4" onSubmit={handleFormSubmit}>
              <label className="block">
                ...
                <input
                  ...
                  className="w-full rounded-xl border-2 border-gray-300 bg-white px-5 py-3 text-sm text-gray-700 shadow-sm transition-all duration-200 focus:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-100"
                />
              </label>
              <div className="flex flex-wrap items-center gap-4 text-xs font-bold uppercase tracking-[0.2em] text-gray-500">
                <span>{labels.filtersLabel}</span>
                <label className="flex items-center gap-2.5 normal-case tracking-normal text-gray-600">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span className="text-xs font-semibold uppercase tracking-[0.15em] text-gray-600">
              ...
              <div className="flex justify-end">
                <button
                  type="submit"
                  className="inline-flex items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-emerald-500/30 transition-all duration-200 hover:shadow-xl hover:shadow-emerald-500/50 hover:-translate-y-0.5"
                >
                  {labels.buttonLabel}
                </button>
              </div>
